/**
 * @file pcm1681.c
 * @brief Driver for the PCM1681 and PCM1681-Q1 Audio DAC
 *
 * @copyright Copyright (c) 2025 Basalte bv
 */
...
{
	return !((reg == PCM1681_FACTORY_REG_0) || (reg == PCM1681_FACTORY_REG_15));
}

static inline bool pcm1681_reg_is_writeable(uint8_t reg)
{
	return pcm1681_reg_is_accessible(reg) && reg != 0x0E;
}
...
static int pcm1681_init(const struct device *dev)
{
	...
	if (ret < 0) {
		LOG_ERR("Failed to apply default propeties (%d)", ret);
		return ret;
	}
